// Taken from https://github.com/AlexeyAB/darknet/blob/master/src/yolo_console_dll.cpp
...
void extrapolate_coords_t::update_result(std::vector<bbox_t> new_result_vec, float new_time, bool update) {
    for (size_t i = 0; i < new_result_vec.size(); ++i) {
        for (size_t k = 0; k < old_result_vec.size(); ++k) {
            if (old_result_vec[k].track_id == new_result_vec[i].track_id &&
                old_result_vec[k].obj_id == new_result_vec[i].obj_id) {
